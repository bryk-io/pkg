...
type Reporter interface {
	...
	// in case of an exception using the `Report` method on the operation
	// instance.
	...
	// Flush should be used to send any buffered events to the tracking
	// server, blocking for at most the given timeout. Must return `false`
	// if the timeout was reached without successfully delivering the
	// error data.
...
type Operation interface {
	...
	// Tags add/update a group of key/value pairs as operation's
	// metadata.
	Tags(tags map[string]interface{})

	// User can be used to declare the user associated with the operation.
	// If used, at least an ID or an IP address should be provided.
	User(usr User)

	// Segment registers additional contextual data under a specific `key`.
	...
	// Report an exception. Usually only unrecoverable errors should be
	// reported at the end of the processing attempt of a given task.
	// This will finish the operation automatically.
	Report(err error) string

	// Status reported on the associated span. Valid values are:
	//  - ok (default)
	//  - error
	//  - canceled
	//  - aborted
	...
	// Finish will mark the operation as completed and trigger the
	// reporting mechanism.
	Finish()
	...
	// Inject set cross-cutting concerns from the operation into the
	// carrier. Allows to propagate operation details across service
	// boundaries.
	Inject(mc Carrier)
}
...
// Carrier elements can be used to transfer cross-cutting concerns about
// an operation across service boundaries.
...
// OperationOption allows to adjust the behavior of error reporting
// operations at the moment of creation.
